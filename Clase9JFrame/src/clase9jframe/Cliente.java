...
public class Cliente extends javax.swing.JFrame {
    ...
    int clienteID = 0;//id unico para cada nacido
    String clientesTotal = " ";//Para el stringBuilder
    ...
    //Guardamos los datos del cliente en varibles mnemotecnicas;
        ...
        //si algun campo esta vacio tira error, sino guarda los dato en la matriz
        if(nombre.isEmpty() || apellido.isEmpty() || edad.isEmpty() || direccion.isEmpty() || codigo.isEmpty()){
        JOptionPane.showMessageDialog(rootPane, "ERROR, Rellene los campos vacios");
        }else{
    ...
    private void btnMostrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnMostrarActionPerformed
        String idBuscado = campo4.getText();//valor de id a buscar
        boolean mensajeNo = false; //bandera

        //iteramos el array comprando el id a buscar con el los ids de los nacidos
        for(int i = 0; i < Cliente.length; i++){
            if(idBuscado.equals(Cliente[i][4])){//si encuentra el id muestra los datos por pantalla
                JOptionPane.showMessageDialog(rootPane, "Nombre: " + Cliente[i][0] + ".\nApellido: " + Cliente[i][1] + ".\nFecha: " + Cliente[i][2] + ".\nLugar: " + Cliente[i][3]);
                    mensajeNo = true;
                    break;
            }
        }
        //si no encuentra el id muestra por pantalla un mensaje de negacion
        if(!mensajeNo){
...
String idAeliminar = campo4.getText();//valor de id a buscar 
boolean mensaje = false; // Bandera
        
        //iteramos el array comprando el id a buscar con el los ids de los nacidos
        for(int i = 0; i < Cliente.length; i++){
            if(idAeliminar.equalsIgnoreCase(Cliente[i][4])){//si encuentra el id reemplaza las siguientes filas del array con sus posteriores
                for(int j = i; j < clienteID - 1; j++){
                    Cliente[j] = Cliente[j+1];
                }
                clienteID--;//reduce en 1 el id
                mensaje = true;
